...
export class GenericFormComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  SetSelectionData(PropertyToSearch: string, SelectedData: any[]) {
    let tempData = this.AllSelectedData.filter((x) => {
      return x.property === PropertyToSearch;
    })[0].SelectedData;
      ...
      for (let field of section.formFieldsSpec) {
        if (field.SelectData) {
          if (!field.PropertyNameToShowInSelection_many) {
            ...
            if (
              field.PropertyNameToShowInSelection_many[
                field.PropertyNameToShowInSelection_many_index_tosearch!
              ].prop === PropertyToSearch
            )
              field.SelectData = [...tempData];
          }
        }
  ...
  selectedChip(
    ...
    let chip = ChipsFromDb.filter((x) => {
      return x[chipPropertyToShowInValue] === event.option.value;
    })[0];
    ...
    for (let section of this.FormSpec.formSections) {
      for (let field of section.formFieldsSpec) {
        if (field.chipsFromDb) {
          if (
            field.chipPropertyToShowInSelection ===
            chipPropertyToShowInSelection
          ) {
            if (event.target.value !== '') {
              field.chipsFromDb = [
                ...field.ChipsFromDbUnFiltered!.filter((i) => {
                  return String(i[chipPropertyToShowInSelection])
                    .toLowerCase()
